      ...
      logo_data.forEach((value,key)=>{
        ...
        <div class="logo__logobox__wrap">
          ...
          <div class="index__logo__text">
            ...
            <p class="index__logo__text__by">by shuu</p>
          </div>
          </a>
        </div>
        `;
...
//検索データ
let _search_data = {"color":[],"improve":[],"type_parent":[],"format":[],"price":[],"improve":[],"type_parent":[],"current_page":[],"per_page":[],"total_page":[],"rine":0};
...
_page_box.on("click",_page_num,(event)=>{
  if($(event.target).hasClass("pagenatoin__num")){
    ...
    // _search_data["current_page"] = current_page;
    search_ajax();
    $(document).scrollTop($("#pageStart").offset().top);
  }
});
...
//並び替え
let _rine = $(".index__logobar__rine__select");

_rine.on("change",(event)=>{
  let rine_num = $(event.target).val();
  _search_data["rine"] = rine_num;
  search_ajax();
});
